...
register_structs! {
    /// HBA registers: generic host control.
    pub Host {
        /// Host Capabilities.
...
// Combined HBA registers.
register_structs! {
    /// Combined HBA registers.
    pub Ctrl {
        (0x0000 => pub ghc:     Host),
        (0x002c => _resvd0:     [u8; 0x60-0x2c]),
        (0x0060 => _resvd1:     [u8; 0xa0-0x60]),
        (0x00a0 => vendor:      [u8; 0x100-0xa0]),
        (0x0100 => pub port:    [Port; 32]),
        (0x1100 => @END),
    }
}
