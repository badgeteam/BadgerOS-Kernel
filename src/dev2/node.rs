...
pub fn add_singleton(name: String, device: Arc<dyn Device>) -> EResult<()> {
    ...
    nodes.insert(name, Nodes::Singleton(Arc::downgrade(&device)));

    Ok(())
}

/// Add a generic device node.
pub fn add(name: String, device: Arc<dyn Device>) -> EResult<()> {
    todo!()
}

/// Add a block device node (e.g. `sataN` and `sataNpM`).
pub fn add_block(name: String, device: Arc<dyn BlockDevice>) -> EResult<()> {
    todo!()
}
